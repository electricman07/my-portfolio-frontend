...
const premiumCardClasses = `
    group relative overflow-hidden flex flex-col h-full bg-[#F8FAFC] border-2 border-slate-300 dark:bg-slate-900 dark:border-slate-600 rounded-[2.5rem] shadow-sm hover:shadow-2xl transition-all duration-500 hover:-translate-y-2
  `;
...
export function ProjectCard({
  ...
  const path =
    typeof image === "string"
      ? image
      : image?.url || image?.data?.attributes?.url || image?.attributes?.url;
  ...
  return (
    <article className={premiumCardClasses}>
      ...
      >
        ...
        <span className="absolute inset-0 flex items-center justify-center  bg-slate-100 dark:bg-slate-900">
          Loading Image...
        </span>
        ...
        <img
          src={imageUrl}
          alt={title}
          className="relative w-full h-full object-cover transition-transform duration-700 ease-in-out group-hover:scale-110"
          onLoad={(e) => {
      ...
      <div className="p-8 flex flex-col grow">
        ...
        >
          <h3 className="text-2xl font-black tracking-tight mb-3 text-slate-950 dark:text-white group-hover/title:text-blue-600 transition-colors">
            {title}
          </h3>
        </Link>
        <p className="text-slate-800 dark:text-slate-400 text-sm line-clamp-3 font-medium leading-relaxed mb-8">
          {description}
        </p>
